...
contextBridge.exposeInMainWorld("botyar", {
  ...
  // ── Projects ────────────────────────────────────────────────────
  projectsList:   () => ipcRenderer.invoke(IPC.projects.list),
  projectsGet:    (id) => ipcRenderer.invoke(IPC.projects.get, id),
  projectsSave:   (input) => ipcRenderer.invoke(IPC.projects.save, input),
  projectsDelete: (id) => ipcRenderer.invoke(IPC.projects.delete, id),

  // ── Sessions ────────────────────────────────────────────────────
  sessionsList:   (projectId) => ipcRenderer.invoke(IPC.sessions.list, projectId),
